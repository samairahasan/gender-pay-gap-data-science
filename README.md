...
The project uses three UK administrative datasets for 2024:

1. **Gender Pay Gap Service (firm-level data)**  
   https://gender-pay-gap.service.gov.uk/Viewing/download  

2. **ONS ASHE: Average Gross Hourly Pay by Industry**  
...
3. **ONS Labour Market Statistics: Female Employment by Industry**  
...
Raw data is included in `data/raw/` to ensure full replicability.  
Raw files are **never modified** — all cleaning scripts read from `data/raw/` and write outputs to `data/clean/`.
...
\[
PayGap_i = \beta_0 + \beta_1 FemaleEmployment_i + \beta_2 AvgWage_i + \varepsilon_i
\]
...
Raw data → Cleaning → Merge → Analysis → Outputs
...
This executes all cleaning, merging, and analysis steps.

Run step-by-step
Shellpython scripts/01_clean_gender_pay_gap.pypython scripts/02_clean_gross_hourly_pay.pypython scripts/03_clean_female_employment.pypython scripts/04_merge.pypython scripts/05_analysis_and_outputs.pyShow more lines
All outputs are generated programmatically from raw data with no manual intervention.

Generated Files

Cleaned datasets: data/clean/
SQLite database: data/uk_gender_pay_gap_industry_2024.db
Figures: output/figures/
Tables: output/tables/
Final analysis/blog: blog/


Use of SQLite
The merged dataset is also stored as a SQLite database:
data/uk_gender_pay_gap_industry_2024.db

This provides a structured, queryable dataset and demonstrates relational data management practices.

Important Data Notes

The final dataset includes 14 industries where all three data sources overlap
In the employment dataset, some SIC categories appear grouped (e.g. "B, D, E" and "R, S, T").
These grouped categories are excluded during cleaning to ensure consistency with single SIC section classification across datasets
This reduces the sample but ensures interpretability and compatibility across sources
...
Results reflect industry-level aggregation and mask within-industry heterogeneity
Female employment measures capture levels, not shares
The analysis uses a relatively small sample size
Findings are descriptive and explanatory, not causal estimates
